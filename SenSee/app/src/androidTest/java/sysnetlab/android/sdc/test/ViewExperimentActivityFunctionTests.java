    ...
    public void testNoteSwipeDirection() {
        ...
        if (loadExperimentWithNotes()) {
            ...
            getInstrumentation().runOnMainSync(new Runnable() {
                @Override
                public void run() {
                    listOperations.performItemClick(
                            listOperations.getAdapter().getView(1, null, null),
            ...
            textView = (TextView) mExperimentViewNotesFragment.getView().findViewById(
                    R.id.textview_fragment_experiment_view_notes_note_caption);
            note1 = Integer.parseInt(String.valueOf(textView.getText().charAt(5)));

            // swipe to the left
            do {
                TouchUtils.dragViewToX(this, mExperimentViewNotesFragment.getView(),
                        Gravity.CENTER, rect.left);
                getInstrumentation().waitForIdleSync();
            ...
            Log.e("Notes missing",
                    "A experiment with a least two notes is required in order to execute this test");
